        ...
        {projects.length === 0 ? (
          ...
          projects.map((project, index) => (
            <motion.article
              ...
              className="group relative overflow-hidden rounded-[18px] border border-deep-ocean/8 bg-white p-6 shadow-[0_2px_16px_rgba(0,0,0,0.06)] transition-shadow duration-300 hover:shadow-[0_8px_32px_rgba(0,0,0,0.1)] will-change-transform"
            >
              {/* Apple-style subtle hover overlay */}
              <div className="pointer-events-none absolute inset-0 rounded-[18px] bg-gradient-to-br from-pharmacy-green/4 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />

              <div className="relative">
                {/* Apple: küçük label + büyük başlık hiyerarşisi */}
                <div className="mb-3 flex items-center gap-2">
                  <span className="h-2 w-2 rounded-full bg-pharmacy-green/70" />
                  <span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-deep-ocean/40">
                    Proje
                  </span>
                </div>
                <h3 className="text-xl font-semibold text-deep-ocean" style={{ letterSpacing: "-0.02em" }}>
                  {project.projectName?.[language] || project.projectName?.tr}
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-deep-ocean/62">
